...
StripCPE::StripCPE(edm::ParameterSet& conf,
  ...
  for (map_t::const_iterator it = modules.begin(); it != modules.end(); it++) {
    ...
    if (!confObj.isParameter(xtalk1S))
      throw cms::Exception("SiStripConfObject does not contain: ") << xtalk1S;
...
StripClusterParameterEstimator::LocalValues StripCPE::localParameters(const SiStripCluster& cluster,
...
float StripCPE::Param::coveredStrips(const LocalVector& lvec, const LocalPoint& lpos) const {
