...
void MultiPrimaryVertexFitter::fill_trackinfo(const std::vector<reco::TransientTrack> &tracks,
  ...
  for (auto &trk : tracks) {
    ...
    double U12 = J12 * J22 * cov(2, 2) + J12 * J21 * cov(1, 2) + J12 * J24 * cov(2, 4) + J13 * J21 * cov(1, 3) +
                 J13 * J22 * cov(3, 2) + J13 * J24 * cov(3, 4);
    // S = U^{-1}
...
void MultiPrimaryVertexFitter::fill_weights(double beta, const reco::BeamSpot &beamspot, const double /* Zcutoff */) {
  ...
  for (unsigned int i = 0; i < nt; i++) {
    ...
    for (unsigned int k = 0; k < nv; k++) {
      ...
      if (chsq < temp_chsqmin[i])
        temp_chsqmin[i] = chsq;
      double chsqz = F2 * F2 * ti.S22;
      if (chsqz < temp_chsqzmin[i])
        temp_chsqzmin[i] = chsqz;
...
void MultiPrimaryVertexFitter::dump(const std::string &comment,
  ...
  for (unsigned int k = 0; k < zv.size(); k++) {
    ...
    if ((zv[k] > zmin) && (zv[k] < zmax)) {
      ...
      if (k < 5) {
        for (unsigned int i = 0; i < trackinfo.size(); i++) {
          ...
          for (const auto &tk : clusters[k].originalTracks()) {
            if (((std::abs(trackinfo[i].z - tk.track().vz()) < 0.00001)) &&
                ((std::abs(trackinfo[i].x - tk.track().vx()) < 0.00001))) {
          ...
          edm::LogPrint("MultiPrimaryVertexFitter") << "  ez= " << std::setw(7) << std::setprecision(4) << tracks[i].track().dzError()
                    ...
                    << "  sxy(b)= " << std::setw(7) << std::setprecision(4)
                    << tracks[i].track().dxy(beamspot) / tracks[i].track().dxyError() << "  sxy(v)= " << std::setw(7)
                    << std::setprecision(4)
                    << tracks[i].track().dxy(math::XYZPoint(xv[k], yv[k], zv[k])) / tracks[i].track().dxyError()
      ...
      if ((hdump < 0) && (((clusters[k].originalTracks().size() > 5) &&
                           ((fabs(xv[k]) > 0.025) || (fabs(yv[k]) > 0.025)) && (fabs(zv[k] - 0.11) < 0.2)) ||
                          (k == 55))) {
        hdump++;
        double z = zv[k];
...
void MultiPrimaryVertexFitter::clean(const std::vector<reco::TransientTrack> &tracks,
                                     const std::vector<TransientVertex> &clusters) {
...
std::vector<TransientVertex> MultiPrimaryVertexFitter::fit(std::vector<reco::TransientTrack> const& tracks,
  ...
  xv.clear();
  xv.reserve(clusters.size());
  yv.clear();
  yv.reserve(clusters.size());
  zv.clear();
  zv.reserve(clusters.size());
  rho_vtx.clear();
  rho_vtx.reserve(clusters.size());
  V_vtx.clear();
  V_vtx.reserve(clusters.size());
  chi2_vtx.clear();
  chi2_vtx.reserve(clusters.size());
  d2Fperp_vtx.clear();
