...
AMultiPrimaryVertexFitter::AMultiPrimaryVertexFitter(const double chicutoff, const double zcutoff,  const double mintrkweight)
...
double AMultiPrimaryVertexFitter::track_in_vertex_chsq(const TrackInfo & ti, const double xvtx, const double yvtx, const double zvtx){
...
void AMultiPrimaryVertexFitter::fill_trackinfo(const std::vector<reco::TransientTrack> &tracks,
  ...
  for (auto &trk : tracks) {
    ...
    if (fabs(DetV) < 1.e-16) {
      edm::LogWarning("AMultiPrimaryVertexFitter") << "Warning, det(V) almost vanishes : " << DetV << " !! This should not happen!" << std::endl;
      ti.S11 = 0;
      ti.S22 = 0;
      ti.S12 = 0;
    } else {
...
AMultiPrimaryVertexFitter::Error3 AMultiPrimaryVertexFitter::get_inverse_beam_covariance(const reco::BeamSpot &beamspot){
    auto SBeam = beamspot.rotatedCovariance3D();
    if (SBeam.Invert()) {
      return SBeam;
    } else {
      edm::LogWarning("AMultiPrimaryVertexFitter") << "Warning, beam-spot covariance matrix inversion failed " << std::endl;
      Error3 S0;
...
std::vector<TransientVertex> AMultiPrimaryVertexFitter::fit(const std::vector<reco::TransientTrack> &tracks,
  ...
  rho_vtx.clear(); rho_vtx.reserve(nv);
  ...
  for (auto &clu : clusters) {
    ...
    const unsigned int  numtk = clu.originalTracks().size() > 0 ? clu.originalTracks().size() : 1;
  ...
  if(nit >= max_iterations){
    edm::LogWarning("AMultiPrimaryVertexFitter") << "iteration limit reached " << nit << "  last delta = " << delta << std::endl
