#ifndef PhysicsTools_L1Scouting_JetMETPerformanceAnalysisDriver_h
#define PhysicsTools_L1Scouting_JetMETPerformanceAnalysisDriver_h
...
class JetMETPerformanceAnalysisDriver : public AnalysisDriverBase {
 ...
 public:
  explicit JetMETPerformanceAnalysisDriver(const std::string& outputFilePath="", const std::string& outputFileMode="recreate");
  explicit JetMETPerformanceAnalysisDriver(const std::string& tfile, const std::string& ttree, const std::string& outputFilePath, const std::string& outputFileMode="recreate");
  virtual ~JetMETPerformanceAnalysisDriver() {}
 ...
 protected:
  ...
  class fillHistoDataJets {
   public:
    ...
    struct Match {
      Match(const std::string& theLabel, const std::string& theJetCollection, const float theJetPtMin, const float theJetPtMax, const float theJetDeltaRMin)
        : label(theLabel), jetCollection(theJetCollection), jetPtMin(theJetPtMin), jetPtMax(theJetPtMax), jetDeltaRMin(theJetDeltaRMin) {}
      std::string label;
      std::string jetCollection;
      float jetPtMin;
      float jetPtMax;
