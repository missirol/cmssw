...
void GeneralHLTOfflineTEST::fillHltMatrix(const std::string & label,
  ...
  if( triggerEventAOD.isValid() && (path.find("HLT_") != std::string::npos) && 
    ...
    for( int iMod=0; iMod<NumModules; iMod++ ){
      ...
      if (modLab.at(0) == '-') { modLab.erase(0, 1); }
      if (modLab.at(0) == '~') { modLab.erase(0, 1); }
	...
	if( bin_num!=1 ){
	  bool passPreviousFilters = true;
	    ...
	    std::string previousFilter(axis->GetBinLabel(ibin));
            if (previousFilter.at(0) == '-') { previousFilter.erase(0, 1); }
            if (previousFilter.at(0) == '~') { previousFilter.erase(0, 1); }

	    edm::InputTag previousModuleWhoseResultsWeWant(previousFilter, "", hltTag);
	    unsigned int idx_previous_module_trg = 0;
	    if( triggerEventAOD.isValid() ) idx_previous_module_trg = triggerEventAOD->filterIndex(previousModuleWhoseResultsWeWant);
    ...
    if (accept && hist_cpfilt_mini) {

      hist_cpfilt_mini->Fill(hist_cpfilt_mini->GetNbinsX()+0.5, 1);
    }
  }
  else{
