...
void EgHLTOfflineClient::runClient_(DQMStore::IBooker& ibooker, DQMStore::IGetter& igetter) {
  ...
  for (auto const& eleHLTFilterName : eleHLTFilterNames_) {
    ...
    for (auto const& region : regions) {
      for (auto const& eleEffTag : eleEffTags_) {
        ...
        //--------------
        createN1EffHists(
            eleHLTFilterName, eleHLTFilterName + "_gsfEle_" + eleEffTag, region, eleN1EffVars_, ibooker, igetter);

        createSingleEffHists(
            eleHLTFilterName, eleHLTFilterName + "_gsfEle_" + eleEffTag, region, eleSingleEffVars_, ibooker, igetter);

        createTrigTagProbeEffHistsNewAlgo(eleHLTFilterName, region, eleTrigTPEffVsVars_, "gsfEle", ibooker, igetter);

        createHLTvsOfflineHists(
            eleHLTFilterName, eleHLTFilterName + "_gsfEle_passFilter", region, eleHLTvOfflineVars_, ibooker, igetter);
      }
    }
  }
  for (auto& filterNr : eleHLTFilterNames2Leg_) {
    for (auto const& region : regions) {
      for (size_t effNr = 0; effNr < eleEffTags_.size(); effNr++) {
        std::string trigNameLeg1 = filterNr.substr(0, filterNr.find("::"));

        std::string trigNameLeg2 = filterNr.substr(filterNr.find("::") + 2);

        ibooker.setCurrentFolder(dirName_ + "/Client_Histos/" + trigNameLeg2);
        createTrigTagProbeEffHists2Leg(
            trigNameLeg1, trigNameLeg2, region, eleTrigTPEffVsVars_, "gsfEle", ibooker, igetter);
      }
    }
  }
  ...
  for (auto const& phoHLTFilterName : phoHLTFilterNames_) {
    for (auto const& region : regions) {
      for (auto const& phoEffTag : phoEffTags_) {
        ...
        createN1EffHists(
            phoHLTFilterName, phoHLTFilterName + "_pho_" + phoEffTag, region, phoN1EffVars_, ibooker, igetter);

        createSingleEffHists(
            phoHLTFilterName, phoHLTFilterName + "_pho_" + phoEffTag, region, phoSingleEffVars_, ibooker, igetter);

        createTrigTagProbeEffHistsNewAlgo(phoHLTFilterName, region, phoTrigTPEffVsVars_, "pho", ibooker, igetter);

        createHLTvsOfflineHists(
            phoHLTFilterName, phoHLTFilterName + "_pho_passFilter", region, phoHLTvOfflineVars_, ibooker, igetter);
  ...
  for (auto const& region : regions) {
    createLooseTightTrigEff(eleTightLooseTrigNames_, region, eleLooseTightTrigEffVsVars_, "gsfEle", ibooker, igetter);

    createLooseTightTrigEff(
        eleTightLooseTrigNames_, region, eleLooseTightTrigEffVsVars_, "gsfEle_trigCuts", ibooker, igetter);

    createLooseTightTrigEff(phoTightLooseTrigNames_, region, phoLooseTightTrigEffVsVars_, "pho", ibooker, igetter);

    createLooseTightTrigEff(
        phoTightLooseTrigNames_, region, phoLooseTightTrigEffVsVars_, "pho_trigCuts", ibooker, igetter);
  }
  //----Morse-----
...
void EgHLTOfflineClient::createHLTvsOfflineHists(const std::string& filterName,
  ...
  */
  ...
  for (auto const& varName : varNames) {
    MonitorElement* numer =
        igetter.get(dirName_ + "/Source_Histos/" + filterName + "/" + baseName + "_HLT" + varName + "_" + region);
    MonitorElement* denom =
        igetter.get(dirName_ + "/Source_Histos/" + filterName + "/" + baseName + "_" + varName + "_" + region);
    if (numer != nullptr && denom != nullptr) {
      std::string effHistName(baseName + "_HLToverOffline_" + varName + "_" +
                              region);  //std::cout<<"hltVSoffline:  "<<effHistName<<std::endl;
      std::string effHistTitle(effHistName);
      if (region == "eb" || region == "ee") {
        if (region == "eb")
          effHistTitle = "Barrel " + baseName + " HLToverOffline " + varName;
        if (region == "ee")
          effHistTitle = "Endcap " + baseName + " HLToverOffline " + varName;
...
void EgHLTOfflineClient::createN1EffHists(const std::string& filterName,
                                          ...
                                          DQMStore::IGetter& igetter) {
  MonitorElement* numer =
      igetter.get(dirName_ + "/Source_Histos/" + filterName + "/" + baseName + "_allCuts_" + region);

  for (auto const& varName : varNames) {
    MonitorElement* denom =
        igetter.get(dirName_ + "/Source_Histos/" + filterName + "/" + baseName + "_n1_" + varName + "_" + region);
    if (numer != nullptr && denom != nullptr) {
      std::string effHistName(baseName + "_n1Eff_" + varName + "_" +
                              region);  //std::cout<<"N1:  "<<effHistName<<std::endl;
      //std::cout<<region<<"  ";
      //----Morse-----------
      std::string effHistTitle(effHistName);  //std::cout<<effHistTitle<<std::endl;
      if (region == "eb" || region == "ee") {
        if (region == "eb")
          effHistTitle = "Barrel " + baseName + " N1eff " + varName;
        if (region == "ee")
          effHistTitle = "Endcap " + baseName + " N1eff " + varName;
...
void EgHLTOfflineClient::createSingleEffHists(const std::string& filterName,
  ...
  for (auto const& varName : varNames) {
    MonitorElement* numer =
        igetter.get(dirName_ + "/Source_Histos/" + filterName + "/" + baseName + "_single_" + varName + "_" + region);
    if (numer != nullptr && denom != nullptr) {
      std::string effHistName(baseName + "_singleEff_" + varName + "_" +
                              region);  //std::cout<<"Si:  "<<effHistName<<std::endl;
      //----Morse-----------
      std::string effHistTitle(effHistName);  //std::cout<<effHistTitle<<std::endl;
      if (region == "eb" || region == "ee") {
        if (region == "eb")
...
void EgHLTOfflineClient::createTrigTagProbeEffHists(const std::string& filterName,
  ...
  for (auto const& vsVarName : vsVarNames) {
    std::string allName(dirName_ + "/Source_Histos/" + filterName + "/" + filterName + "_trigTagProbe_" + objName +
                        "_all_" + vsVarName + "_" + region);
    MonitorElement* all = igetter.get(allName);
    if (all == nullptr) {
      continue;
    }
    std::string passName(dirName_ + "/Source_Histos/" + filterName + "/" + filterName + "_trigTagProbe_" + objName +
                         "_pass_" + vsVarName + "_" + region);
    MonitorElement* pass = igetter.get(passName);
    if (pass == nullptr) {
      continue;
    }
    //----Morse-----
    std::string effHistTitle(filterName + "_trigTagProbeEff_" + objName + "_vs_" + vsVarName + "_" +
                             region);  //std::cout<<effHistTitle<<std::endl;
    if (region == "eb" || region == "ee") {
      if (region == "eb")
        effHistTitle = "Barrel " + filterName + "_" + objName + " TrigTagProbeEff vs " + vsVarName;
      if (region == "ee")
        effHistTitle = "Endcap " + filterName + "_" + objName + " TrigTagProbeEff vs " + vsVarName;
    }
    //------------
...
void EgHLTOfflineClient::createTrigTagProbeEffHistsNewAlgo(const std::string& filterName,
  ...
  for (auto const& vsVarName : vsVarNames) {
    ...
    std::string passName(dirName_ + "/Source_Histos/" + filterName + "/" + filterName + "_trigTagProbe_" + objName +
                         "_passNotTag_" + vsVarName + "_" + region);
    ...
    std::string passTagTagName(dirName_ + "/Source_Histos/" + filterName + "/" + filterName + "_trigTagProbe_" +
                               objName + "_passTagTag_" + vsVarName + "_" + region);
    ...
    std::string failName(dirName_ + "/Source_Histos/" + filterName + "/" + filterName + "_trigTagProbe_" + objName +
                         "_fail_" + vsVarName + "_" + region);
...
void EgHLTOfflineClient::createTrigTagProbeEffHists2Leg(const std::string& filterNameLeg1,
                                                        ...
                                                        DQMStore::IGetter& igetter) {
  for (auto const& vsVarName : vsVarNames) {
    std::string allName(dirName_ + "/Source_Histos/" + filterNameLeg2 + "/" + filterNameLeg2 + "_trigTagProbe_" +
                        objName + "_all_" + vsVarName + "_" + region);
    MonitorElement* all = igetter.get(allName);
    if (all == nullptr) {
      edm::LogInfo("EgHLTOfflineClient") << " couldnt get hist " << allName;
      continue;
    ...
    std::string Leg2NotLeg1SourceName(dirName_ + "/Source_Histos/" + filterNameLeg2 + "/" + filterNameLeg2 +
                                      "_trigTagProbe_" + objName + "_passLeg2failLeg1_" + vsVarName + "_" + region);
    ...
    std::string Leg1EffName(dirName_ + "/Client_Histos/" + filterNameLeg1 + "/" + filterNameLeg1 + "_trigTagProbeEff_" +
                            objName + "_vs_" + vsVarName + "_" + region);
    ...
    std::string effHistTitle(filterNameLeg2 + "_trigTagProbeEff2Leg_" + objName + "_vs_" + vsVarName + "_" +
                             region);  //std::cout<<effHistTitle<<std::endl;
    if (region == "eb" || region == "ee") {
      if (region == "eb")
        effHistTitle = "Barrel " + filterNameLeg2 + "_" + objName + " TrigTagProbeEff2Leg vs " + vsVarName;
      if (region == "ee")
        effHistTitle = "Endcap " + filterNameLeg2 + "_" + objName + " TrigTagProbeEff2Leg vs " + vsVarName;
    }  //std::cout<<effHistTitle<<std::endl;
...
void EgHLTOfflineClient::createLooseTightTrigEff(const std::vector<std::string>& tightLooseTrigNames,
  ...
  for (auto const& vsVarName : vsVarNames) {
    for (auto const& tightLooseTrigName : tightLooseTrigNames) {
      ...
      MonitorElement* fail = igetter.get(dirName_ + "/Source_Histos/" + tightTrig + "_" + looseTrig + "_" + objName +
                                         "_failTrig_" + vsVarName + "_" + region);
      if (fail == nullptr) {
        continue;
      }

      MonitorElement* pass = igetter.get(dirName_ + "/Source_Histos/" + tightTrig + "_" + looseTrig + "_" + objName +
                                         "_passTrig_" + vsVarName + "_" + region);
      if (pass == nullptr) {
        continue;
      }

      const std::string newHistName(tightTrig + "_trigEffTo_" + looseTrig + "_" + objName + "_vs_" + vsVarName + "_" +
                                    region);
      //----Morse-----
      std::string effHistTitle(newHistName);  //std::cout<<effHistTitle<<std::endl;
      if (region == "eb" || region == "ee") {
        if (region == "eb")
          effHistTitle = "Barrel " + tightTrig + "_TrigEffTo_" + looseTrig + "_" + objName + " vs " + vsVarName;
        if (region == "ee")
          effHistTitle = "Endcap " + tightTrig + "_TrigEffTo_" + looseTrig + "_" + objName + " vs " + vsVarName;
      }
      //------------
