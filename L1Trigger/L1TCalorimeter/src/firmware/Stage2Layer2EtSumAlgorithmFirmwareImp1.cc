    ...
    for (unsigned absieta = 1; absieta <= (unsigned int)CaloTools::mpEta(CaloTools::kHFEnd); absieta++) {
      ...
      if (!params_->etSumBypassEttPUS()) {
        if (params_->etSumEttPUSType() == "LUT") {
          ...
          if (params_->etSumEttPUSType() != "None" && params_->etSumEttPUSType() != "none") {
            edm::LogError("l1t|stage 2") << "Invalid PUS type in calo params. Not applying PUS to Stage 2 ETT"
                                         << std::endl;
            return;
          }
        }
      ...
      if (!params_->etSumBypassEcalSumPUS()) {
        if (params_->etSumEcalSumPUSType() == "LUT") {
          ...
          if (params_->etSumEcalSumPUSType() != "None" && params_->etSumEcalSumPUSType() != "none") {
            edm::LogError("l1t|stage 2") << "Invalid PUS type in calo params. Not applying PUS to Stage 2 Ecal ETT"
                                         << std::endl;
            return;
          }
        }
      ...
      for (int iphi = 1; iphi <= CaloTools::kHBHENrPhi; iphi++) {
        ...
        if (tower.hwPt() > towEtMetThresh_ && CaloTools::mpEta(abs(tower.hwEta())) <= CaloTools::mpEta(metEtaMax_) &&
          ...
          if (tower.hwPt() == CaloTools::kSatHcal || tower.hwPt() == CaloTools::kSatEcal ||
              tower.hwPt() == CaloTools::kSatTower)
            metSat = true;
        ...
        if (tower.hwPt() > towEtSumEtThresh_ &&
            CaloTools::mpEta(abs(tower.hwEta())) <= CaloTools::mpEta(ettEtaMaxHF_) && !ettHFSat) {
          if (tower.hwPt() == CaloTools::kSatHcal || tower.hwPt() == CaloTools::kSatEcal ||
              tower.hwPt() == CaloTools::kSatTower)
            ettHFSat = true;
          ringEtHF += tower.hwPt();
        ...
        // scalar sum (EM)
        if (tower.hwPt() > towEtEcalSumThresh_ &&
            CaloTools::mpEta(abs(tower.hwEta())) <= CaloTools::mpEta(ettEtaMax_) && !ecalEtSat) {
          if (tower.hwPt() == CaloTools::kSatEcal || tower.hwPt() == CaloTools::kSatTower)
            ecalEtSat = true;
...
edm::LogPrint("AAA") << " XXX2 (MB0 = " << mb0 << ", MB1 = " << mb1 << ")";
    ...
    if (ettHFSat || etHF > 0xffff)
      etHF = 0xffff;
    if (metSat || ex > 0x7fffffff)
      ex = 0x7fffffff;
    if (metSat || ey > 0x7fffffff)
      ey = 0x7fffffff;
    if (metHFSat || exHF > 0x7fffffff)
      exHF = 0x7fffffff;
    if (metHFSat || eyHF > 0x7fffffff)
      eyHF = 0x7fffffff;
