  ...
  for (size_t jj = 0; jj < cuts.size(); jj++) {
    ...
    if (cut.getCutType() == esCutType::ChargeCorrelation) {
      ...
      if (cut.getCutType() == esCutType::DeltaEta) {
        ...
        corrParameter.minEtaCutValue = multiply_and_clamp_longlong(minV, pow(10., cut.getMinimum().index));
        corrParameter.maxEtaCutValue = multiply_and_clamp_longlong(maxV, pow(10., cut.getMaximum().index));
        ...
        LogDebug("TriggerMenuParser") << "CutType: " << cut.getCutType() << "\tDeltaPhi Cut minV = " << minV
                                      << " Max = " << maxV << " precMin = " << cut.getMinimum().index
                                      << " precMax = " << cut.getMaximum().index << std::endl;
        corrParameter.minPhiCutValue = multiply_and_clamp_longlong(minV, pow(10., cut.getMinimum().index));
        corrParameter.maxPhiCutValue = multiply_and_clamp_longlong(maxV, pow(10., cut.getMaximum().index));
        ...
        LogDebug("TriggerMenuParser") << "CutType: " << cut.getCutType() << "\tDeltaR Cut minV = " << minV
                                      << " Max = " << maxV << " precMin = " << cut.getMinimum().index
                                      << " precMax = " << cut.getMaximum().index << std::endl;
        corrParameter.minDRCutValue = multiply_and_clamp_longlong(minV, pow(10., cut.getMinimum().index));
        corrParameter.maxDRCutValue = multiply_and_clamp_longlong(maxV, pow(10., cut.getMaximum().index));
        corrParameter.precDRCut = cut.getMinimum().index;
        cutType = cutType | 0x4;
      } else if (cut.getCutType() == esCutType::TwoBodyPt) {
        corrParameter.minTBPTCutValue = multiply_and_clamp_longlong(minV, pow(10., cut.getMinimum().index));
        corrParameter.maxTBPTCutValue = multiply_and_clamp_longlong(maxV, pow(10., cut.getMaximum().index));
        ...
        LogDebug("TriggerMenuParser") << "CutType: " << cut.getCutType() << "\tMass Cut minV = " << minV
                                      << " Max = " << maxV << " precMin = " << cut.getMinimum().index
                                      << " precMax = " << cut.getMaximum().index << std::endl;
        corrParameter.minMassCutValue = multiply_and_clamp_longlong(minV, pow(10., cut.getMinimum().index));
        corrParameter.maxMassCutValue = multiply_and_clamp_longlong(maxV, pow(10., cut.getMaximum().index));
  ...
  for (size_t lll = 0; lll < cuts.size(); lll++) {  // START esCut lll
    ...
    if (cut.getCutType() == esCutType::Mass) {
      LogDebug("TriggerMenuParser") << "CutType: " << cut.getCutType() << "\tMass Cut minV = " << minV
                                    ...
                                    << " precMax = " << cut.getMaximum().index << std::endl;
      corrThreeBodyParameter.minMassCutValue = multiply_and_clamp_longlong(minV, pow(10., cut.getMinimum().index));
      corrThreeBodyParameter.maxMassCutValue = multiply_and_clamp_longlong(maxV, pow(10., cut.getMaximum().index));
    ...
    if (cut.getCutType() == esCutType::ChargeCorrelation) {
      ...
      if (cut.getCutType() == esCutType::OvRmDeltaEta) {
        ...
        corrParameter.minOverlapRemovalEtaCutValue =
            multiply_and_clamp_longlong(minV, pow(10., cut.getMinimum().index));
        corrParameter.maxOverlapRemovalEtaCutValue =
            multiply_and_clamp_longlong(maxV, pow(10., cut.getMaximum().index));
        ...
        //std::cout << "DeltaR Cut minV = " << minV << " Max = " << maxV << " precMin = " << cut.getMinimum().index << " precMax = " << cut.getMaximum().index << std::endl;
        corrParameter.minOverlapRemovalDRCutValue = multiply_and_clamp_longlong(minV, pow(10., cut.getMinimum().index));
        corrParameter.maxOverlapRemovalDRCutValue = multiply_and_clamp_longlong(maxV, pow(10., cut.getMaximum().index));
...
long long l1t::TriggerMenuParser::multiply_and_clamp_longlong(double const d1, double const d2) const {
  double const prod = d1 * d2;

  if (edm::isNotFinite(prod)) {
    throw cms::Exception("InvalidValue") << "l1t::TriggerMenuParser::multiply_and_clamp_longlong(" << d1 << ", " << d2
                                         << "): product of input values (" << prod << ") is not a finite number !";
  }

  return std::clamp(prod, kMinLongLongAsDouble, kMaxLongLongAsDouble);
}
