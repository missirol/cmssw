// -*- C++ -*-
//
// Package:    GeneralHLTOfflineTEST
// Class:      GeneralHLTOfflineTEST
//
/**\class GeneralHLTOfflineTEST
...
class GeneralHLTOfflineTEST : public DQMEDAnalyzer {
 public:
  explicit GeneralHLTOfflineTEST(const edm::ParameterSet&);
  ~GeneralHLTOfflineTEST() override;
...
//
GeneralHLTOfflineTEST::GeneralHLTOfflineTEST(const edm::ParameterSet& ps):streamA_found_(false),
                                                                  hlt_menu_(""),
                                                                  cppath_(nullptr) {
  debugPrint  = false;
  outputPrint = false;
...
GeneralHLTOfflineTEST::~GeneralHLTOfflineTEST() = default;

// ------------ method called for each event  ------------
void
GeneralHLTOfflineTEST::analyze(const edm::Event& iEvent,
  ...
  if (streamA_found_) {
	...
	if (debugPrint) {
        ...
        std::string fullPathToCPP = "HLT/GeneralHLTOfflineTEST/"
...
void
GeneralHLTOfflineTEST::dqmBeginRun(edm::Run const& iRun,
...
void GeneralHLTOfflineTEST::bookHistograms(DQMStore::IBooker & iBooker,
				       edm::Run const & iRun,
				       edm::EventSetup const & iSetup)
{
  ...
  iBooker.setCurrentFolder("HLT/GeneralHLTOfflineTEST/");
...
void GeneralHLTOfflineTEST::setupHltMatrix(DQMStore::IBooker & iBooker, const std::string & label, int iPD) {
  std::string h_name;
  std::string h_title;
  std::string pathName;
  ...
  PD_Folder = "HLT/GeneralHLTOfflineTEST/" + label;
  ...
  // paths
  std::string folderz;
  folderz = "HLT/GeneralHLTOfflineTEST/" + label;
...
void GeneralHLTOfflineTEST::fillHltMatrix(const std::string & label,
  ...
  fullPathToCPP = "HLT/GeneralHLTOfflineTEST/" + label + "/cppath_" + label + "_" + hlt_menu_;
  ...
  if(  (path.find("HLT_") != std::string::npos) && 
    ...
    else if( hasRawTriggerEvent && triggerEventRAW.isValid() ) triggerEventSize = triggerEventRAW->size();
...
//!!
//!!if(path.find("HLT_IsoMu24_v") != std::string::npos){
//!! std::cout<<"---------\n";
//!! std::cout<<"label " << label << "\n";
//!! std::cout<<"path " << path << "\n";
//!! std::cout<<"hasRawTriggerEvent " << hasRawTriggerEvent << "\n";
//!! std::cout<<"triggerEventRAW.isValid() " << triggerEventRAW.isValid() << "\n";
//!! if( hasRawTriggerEvent && triggerEventRAW.isValid() ){
//!!
//!!   for(uint idx=0; idx<triggerEventRAW->size(); ++idx){
//!!
//!!     std::cout << "triggerEventRAW.filterTag("<<idx<<").encode() = " << triggerEventRAW->filterTag(idx).encode() << "\n";
//!!   }
//!! }
//!! if( triggerEventAOD.isValid() ){
//!!
//!!   for(uint idx=0; idx<triggerEventAOD->sizeFilters(); ++idx){
//!!
//!!     std::cout << "triggerEventAOD.filterTag("<<idx<<").encode() = " << triggerEventAOD->filterTag(idx).encode() << "\n";
//!!   }
//!! }
//!! std::cout<<"---------\n";
//!!}
//!!
    ...
    const auto& triggerObjects = triggerEventAOD->getObjects();

    for( int iMod=0; iMod<NumModules; iMod++ ){

      bool ignoreFilterRes(false);
      bool invertFilterRes(false);

      auto modLab = moduleLabels[iMod];
      if (modLab.at(0) == '-') { modLab.erase(0, 1); ignoreFilterRes = true; }
      if (modLab.at(0) == '~') { modLab.erase(0, 1); invertFilterRes = true; }

      edm::InputTag moduleWhoseResultsWeWant(modLab, "", hltTag);
      ...
      else if( hasRawTriggerEvent && triggerEventRAW.isValid() ) idx_module_trg = triggerEventRAW->filterIndex(moduleWhoseResultsWeWant);
...
//!!        const trigger::Keys& keys = triggerEventAOD->filterKeys(idx_module_trg);
...
//!!	return (leadingPt > 0.0);
        ...
        const bool filterRes = true; //!! invertFilterRes ? (not ) : ;

        if(ignoreFilterRes || filterRes){
...
//
////!!	    bool ignorePreviousFilter(false);
////!!            bool invertPreviousFilter(false);
////!!            if (previousFilter.at(0) == '-') {
////!!
////!!              ignorePreviousFilter = true;
////!!              previousFilter.erase(0, 1);
////!!	    }
////!!        else if (previousFilter.at(0) == '~') {
////!!
////!!	      invertPreviousFilter = true;
////!!              previousFilter.erase(0, 1);
////!!	    }
////!!
////!!            if(ignorePreviousFilter){ continue; }
//
//            if (previousFilter.at(0) == '-') { continue; }//!!
//
...
DEFINE_FWK_MODULE(GeneralHLTOfflineTEST);
