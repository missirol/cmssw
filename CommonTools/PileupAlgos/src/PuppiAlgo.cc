...
void PuppiAlgo::computeMedRMS(const uint iAlgo) {
  ...
  int lNHalfway = lNBefore + lNum0 + int((fNCount[iAlgo] - lNum0) * 0.5f);
  fMedian[iAlgo] = fPups[lNHalfway];
  float lMed = fMedian[iAlgo];  //Just to make the readability easier
...
float PuppiAlgo::compute(std::vector<float> const &iVals, const float iChi2) const {
  if (fAlgoId[0] == -1)
    return 1;
  float lVal = 0.;
  float lPVal = 1.;
