...
void PuppiContainer::initialize(const std::vector<RecoObj> &iRecoObjects) {
  ...
  for (auto const &rParticle : *fRecoParticles) {
    ...
    PuppiCandidate rObj;
    rObj.pt = curPseudoJet.pt();
    rObj.eta = curPseudoJet.eta();
    rObj.rapidity = curPseudoJet.rap();
    rObj.phi = curPseudoJet.phi();
    rObj.m = curPseudoJet.m();
    rObj.id = rParticle.id;

    fPFParticles.push_back(rObj);
    //Take Charged particles associated to PV
    if (std::abs(rParticle.id) == 3)
      continue;
    fPFParticlesForVar.push_back(rObj);
    if (std::abs(rParticle.id) == 1)
      fPFParticlesForVarChargedPV.push_back(rObj);
    //if(rParticle.id == 3) _chargedNoPV.push_back(rObj);
