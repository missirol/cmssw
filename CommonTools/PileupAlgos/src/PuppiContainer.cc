...
void PuppiContainer::initialize(std::vector<PuppiCandidate> const& iPuppiCandidates, int const iNPV) {
  //Clear everything
  fChargedPV.clear();
  fWeights.clear();
  fVals.clear();
  ...
  int lNParticles = iPuppiCandidates.size();
  ...
  for (int i0 = 0; i0 < lNMaxAlgo; i0++) {
    getRMSAvg(i0, iPuppiCandidates, fChargedPV);
  }
  if (fPuppiDiagnostics)
    getRawAlphas(0, iPuppiCandidates, fChargedPV);
  ...
  for (int i0 = 0; i0 < lNParticles; i0++) {
    ...
    const auto &rParticle = iPuppiCandidates.at(i0);
...
void PuppiContainer::getRMSAvg(int const iOpt,
  ...
  for (unsigned int i0 = 0; i0 < iParticles.size(); i0++) {
    ...
    if (!edm::isFinite(pVal)) {
      LogDebug("NotFound") << "====> Value is Nan " << pVal << " == " << iPart.pt << " -- "
                           << iPart.eta;
      continue;
    }
