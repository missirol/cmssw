...
private:
  ...
  std::unordered_map<unsigned int, const std::pair<double, int>> trigPreList_;
  bool changed_;
  double pLimits_[6];
      ...
      for (unsigned int in = 0; in < trigNames_.size(); ++in) {
        ...
        if (triggerNames_[i].find(trigNames_[in]) != std::string::npos) {
          ...
          if (hlt > 0) {
            ...
            auto const prescales = hltPrescaleProvider_.prescaleValues<double, int>(iEvent, iSetup, triggerNames_[i]);
...
void IsoTrig::endJob() {
  ...
  for (auto const &[runNum, nAccept] : trigList_) {
    auto const &itrPre = trigPreList_[runNum];
    auto const preL1 = itrPre.first;
    auto const preHLT = itrPre.second;
