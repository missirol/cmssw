...
class OrbitNanoAODRNTupleOutputModule : public edm::one::OutputModule<> {
public:
  OrbitNanoAODRNTupleOutputModule(edm::ParameterSet const& pset);
  ~OrbitNanoAODRNTupleOutputModule() override = default;
...
private:
  ...
  public:
    void createFields(RNTupleModel& model) {
      ...
      m_bunchCrossing = model.MakeField<UInt_t>("bunchCrossing");
      m_orbitNumber = model.MakeField<std::uint64_t>("orbitNumber");
    }
    void fill(const edm::EventID& id) {
      *m_run = id.run();
      *m_luminosityBlock = id.luminosityBlock();
      *m_orbitNumber = id.event();
    }
  ...
  private:
    ...
    std::shared_ptr<UInt_t> m_bunchCrossing;
    std::shared_ptr<std::uint64_t> m_orbitNumber;
  } m_commonFields;
...
OrbitNanoAODRNTupleOutputModule::OrbitNanoAODRNTupleOutputModule(edm::ParameterSet const& pset)
...
void OrbitNanoAODRNTupleOutputModule::writeLuminosityBlock(edm::LuminosityBlockForOutput const& iLumi) {
...
void OrbitNanoAODRNTupleOutputModule::writeRun(edm::RunForOutput const& iRun) {
  edm::Service<edm::JobReport> jr;
...
bool OrbitNanoAODRNTupleOutputModule::isFileOpen() const { return nullptr != m_ntuple.get(); }

void OrbitNanoAODRNTupleOutputModule::openFile(edm::FileBlock const&) {
...
void OrbitNanoAODRNTupleOutputModule::initializeNTuple(edm::EventForOutput const& iEvent) {
  ...
  for (const auto& keep : keeps[edm::InEvent]) {
    if (keep.first->className() == "l1ScoutingRun3::OrbitFlatTable") {
      edm::Handle<nanoaod::OrbitFlatTable> handle;
      ...
      throw cms::Exception("Configuration", "OrbitNanoAODRNTupleOutputModule cannot handle class " + keep.first->className());
    }
  }
  m_tables.createFields(iEvent, *model);
  m_selbxs.createFields(iEvent, *model);
...
void OrbitNanoAODRNTupleOutputModule::write(edm::EventForOutput const& iEvent) {
  if (!m_ntuple) {
  ...
  m_selbxs.fill(iEvent);
  m_ntuple->Fill();

  ++m_nOrbits;
}

void OrbitNanoAODRNTupleOutputModule::reallyCloseFile() {
...
void OrbitNanoAODRNTupleOutputModule::writeProvenance() {
  PSetNTuple pntuple;
...
void OrbitNanoAODRNTupleOutputModule::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(OrbitNanoAODRNTupleOutputModule);
