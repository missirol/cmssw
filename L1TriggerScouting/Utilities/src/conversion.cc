...
float l1ScoutingRun3::calol1::fEta(int16_t hwEta) {
  ...
  //
  static constexpr float kTowerEtas[43] = {
  ...
  uint8_t const absHwEta = std::min(42, std::abs(hwEta));
  return (hwEta < 0) ? -kTowerEtas[absHwEta] : kTowerEtas[absHwEta];
}
...
float l1ScoutingRun3::calol1::fPhi(int16_t hwPhi) {
  ...
  //  L1Trigger/L1TCalorimeter/src/CaloTools.cc
  //
  static constexpr uint8_t kNPhi = 72;
  float const phi = (2 * hwPhi - 1) * M_PI / kNPhi;
