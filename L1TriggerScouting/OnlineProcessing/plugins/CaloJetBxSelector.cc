...
private:
  ...
  edm::EDGetTokenT<OrbitCollection<l1ScoutingRun3::CaloJet>> const jetsToken_;
...
CaloJetBxSelector::CaloJetBxSelector(const edm::ParameterSet& iPSet)
  ...
  if (minNJet_ != minJetPt_.size()) {
    throw cms::Exception("InvalidConfiguration")
        << "invalid configuration parameters: \"minNJet\" (" << minNJet_ << ") differs from the size of \"minJetPt\" ("
        << minJetPt_.size() << ")";
  }

  if (minNJet_ != maxJetAbsEta_.size()) {
    throw cms::Exception("InvalidConfiguration")
        << "invalid configuration parameters: \"minNJet\" (" << minNJet_
        << ") differs from the size of \"maxJetAbsEta\" (" << maxJetAbsEta_.size() << ")";
  }

  if (minNJet_ != minJetNConst_.size()) {
    throw cms::Exception("InvalidConfiguration")
        << "invalid configuration parameters: \"minNJet\" (" << minNJet_
        << ") differs from the size of \"minJetNConst\" (" << minJetNConst_.size() << ")";
  }

  produces<std::vector<unsigned>>("SelBx").setBranchAlias("CaloJetSelectedBx");
}
...
void CaloJetBxSelector::produce(edm::StreamID, edm::Event& iEvent, const edm::EventSetup&) const {
  ...
  for (auto const bx : caloJetsInOrbit.getFilledBxs()) {
    if (minNJet_ > 0) {
      ...
      // it must be in a certain eta region, passing cuts on min pT and n-constituents
      unsigned int nAccJets{0};
      for (auto const& jet : jets) {
        // found jet meeting requirements
