...
public:
  MeasurementDetWithData() : det_(nullptr), data_(nullptr) {}

  MeasurementDetWithData(const MeasurementDet& det, const MeasurementTrackerEvent& data) : det_(&det), data_(&data) {}
  ...
  RecHitContainer recHits(const TrajectoryStateOnSurface& tsos) const { return mdet().recHits(tsos, data()); }
