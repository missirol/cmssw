...
public:
  explicit MaxCCCLostHitsTrajectoryFilter(int maxCCCHits = 0, float CCC_value = 0)
      : theMaxCCCLostHits_(maxCCCHits), minGoodStripCharge_(CCC_value) {}
...
protected:
  ...
  bool TBC(T& traj) const {
    ...
    if (!ret)
      traj.setStopReason(StopReason::MAX_CCC_LOST_HITS);
    return ret;
  }
