...
public:
  ...
  StraightLinePropagator* clone() const override { return new StraightLinePropagator(*this); }
...
private:
  ...
  bool propagateParametersOnCylinder(
      const FreeTrajectoryState& fts, const Cylinder& cylinder, GlobalPoint& x, GlobalVector& p, double& s) const;

  // compute propagated x and p and path s, return true when propagation is OK
  bool propagateParametersOnPlane(
      const FreeTrajectoryState& fts, const Plane& plane, LocalPoint& x, LocalVector& p, double& s) const;
};
