...
HLTEcalResonanceFilter::HLTEcalResonanceFilter(const edm::ParameterSet &iConfig)
  ...
  if (doSelBarrel_) {
    ...
    if (seleBeltDR <= 0) {
      throw cms::Exception("InvalidParameterValue")
          << "Invalid value for \"seleBeltDR\" (" << seleBeltDR << "): it must be greater than zero.";
    }
    seleBeltDR2_ = seleBeltDR * seleBeltDR;
  ...
  if (doSelEndcap_) {
    ...
    if (seleBeltDREndCap <= 0) {
      throw cms::Exception("InvalidParameterValue")
          << "Invalid value for \"seleBeltDREndCap\" (" << seleBeltDREndCap << "): it must be greater than zero.";
    }
    seleBeltDR2EndCap_ = seleBeltDREndCap * seleBeltDREndCap;
