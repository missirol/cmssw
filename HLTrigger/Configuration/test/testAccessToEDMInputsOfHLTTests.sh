...
for inputFile in $(cat "${inputFileList}"); do
  cmsRun "${TESTDIR}"/testAccessToEDMInputsOfHLTTests_cfg.py inputFiles="${inputFile}" &>> log_testAccessToEDMInputsOfHLTTests \
    || die "Failure running testAccessToEDMInputsOfHLTTests_cfg.py" $? log_testAccessToEDMInputsOfHLTTests
done
unset inputFile
