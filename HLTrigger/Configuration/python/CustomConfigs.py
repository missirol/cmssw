...
def customiseGlobalTagForOnlineBeamSpot(process):
    """Customisation of GlobalTag for Online BeamSpot
       - edits GlobalTag ESSource to load the records needed to produce the HLT beamspot
       - these records are not available in the Offline GT, which is the GT presently used in HLT+RECO tests
       - not adding these records to the configuration may not result in an error,
         but might lead to an HLT beamspot different to the one obtained when running HLT alone
    """
