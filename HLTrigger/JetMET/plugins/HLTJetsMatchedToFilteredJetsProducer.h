...
template <typename TriggerJetsType, typename TriggerJetsRefType>
HLTJetsMatchedToFilteredJetsProducer<TriggerJetsType, TriggerJetsRefType>::HLTJetsMatchedToFilteredJetsProducer(edm::ParameterSet const& iConfig) : jetsToken_(consumes(iConfig.getParameter<edm::InputTag>("src"))), triggerJetsToken_(consumes(iConfig.getParameter<edm::InputTag>("triggerJetsFilter"))), triggerJetsType_(iConfig.getParameter<int>("triggerJetsType")), maxDeltaR_(iConfig.getParameter<double>("maxDeltaR")), maxDeltaR2_(maxDeltaR_ * maxDeltaR_) {
  if (maxDeltaR_ <= 0.) {
...
template <typename TriggerJetsType, typename TriggerJetsRefType>
void HLTJetsMatchedToFilteredJetsProducer<TriggerJetsType, TriggerJetsRefType>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
template <typename TriggerJetsType, typename TriggerJetsRefType>
void HLTJetsMatchedToFilteredJetsProducer<TriggerJetsType, TriggerJetsRefType>::produce(edm::StreamID, edm::Event& iEvent, edm::EventSetup const&) const {
  ...
  for (auto const& jet_i : jets) {
    for (auto const& jetRef_j : triggerJetsRefVec) {
      auto const dR2 = reco::deltaR2(jet_i.p4(), jetRef_j->p4());
      if (dR2 < maxDeltaR2_) {
