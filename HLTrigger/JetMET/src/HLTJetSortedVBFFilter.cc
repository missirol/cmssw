...
//
template <typename T>
HLTJetSortedVBFFilter<T>::HLTJetSortedVBFFilter(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
...
template <typename T>
bool HLTJetSortedVBFFilter<T>::hltFilter(edm::Event& event,
  ...
  if (inputJetTags_.encode().empty()) {
    ...
    float csv_max = -999;
    for (typename TCollection::const_iterator jet = jets->begin(); (jet != jets->end() && nJet < nMax);
         ++jet) {  //fill "sorted" and get the most b-tagged jet with higher CSV (b1)
    ...
    unsigned int i = 0;
    while ((i == q1_idx) || (i == q2_idx) || (i == b1_idx))
      i++;  //take jet with highest pT but q1,q2,b1 (q2)
    unsigned int b2_idx = i;

    if (q1_idx < jets->size())
      q1 = jets->at(q1_idx).p4();
    else
      edm::LogWarning("Something wrong with q1");
    if (q2_idx < jets->size())
      q2 = jets->at(q2_idx).p4();
    else
      edm::LogWarning("Something wrong with q2");
    if (b1_idx < jets->size())
      b1 = jets->at(b1_idx).p4();
    else
      edm::LogWarning("Something wrong with b1");
    if (b2_idx < jets->size())
      b2 = jets->at(b2_idx).p4();
    else
      edm::LogWarning("Something wrong with b2");
    ...
    float csv2 = -999;
    for (typename TCollection::const_iterator jet = jets->begin(); (jet != jets->end() && nJet < nMax);
         ++jet) {  //fill "sorted" and get the two most b-tagged jets (b1,b2)
      value = findCSV(jet, *jetTags);
      if (value > csv1) {
        csv2 = csv1;
        b2_idx = b1_idx;
  ...
  if ((mqq_bs > mqq_) && (deltaetaqq > detaqq_) && (deltaetabb < detabb_) && (deltaphibb < dphibb_) &&
      (ptsqq_bs > ptsqq_) && (ptsbb_bs > ptsbb_) && (signeta < seta_)) {
    accept = true;
